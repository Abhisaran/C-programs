...
#include<string.h>
#include<process.h>
int bubble(int*,int);/*to bubble sort*/
void filewrite();
void avgmarks();
void fileprint();
void filesort();     /*to call sort*/
void rollin();	     /*to find student by roll no*/
/*********************** SORTING FUNCTION ***************************/
int bubble(int x[],int n)
	{
	 int temp,j,pass,i,switched=1;
...
void main()
	{
	 int marks[100],rollno[100],x[100],n,i,j,roll,c,mark,roll1,buf,k,chz;
	 char name[100][10],nam[50],zak[10]="krce",usr[20],pswrd[10];
	 clrscr();
	 printf("Enter the username :");
	 scanf("%s",usr);
	 printf("Enter the password:");
	 scanf("%s",pswrd);
	 buf=strcmp(zak,pswrd);
	 if(buf!=0)
		{
		 exit(1);
		 }
		 while(c!=6)
		 {
		  ...
		  printf("   2)To see student database.\n");
		  printf("   3)To sort students on basis of marks.\n");
		  printf("   4)To search student using roll no.\n");
		  printf("   5)To find the average mark.\n");
		  printf("   6)To exit.\n\n-");
		  scanf("%d",&c);
		  clrscr();
		  switch(c)
